...
function searchPicker() {
  Inertia.get(route("centers.index", { search: this.search }));
}
...
function downloadExcelFile() {
  axios({
    ...
    .then((response) => {
      window.location = "/centers/download?fileLocation=" + response.data;
    })
    .catch((err) => {
      console.log(err);
    });
    ...
    <div class="">
      <div class="sm:flex sm:items-center">
        ...
        <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
          <Button>
            <Link href="/centers/create">Add New</Link>
          </Button>
        </div>
      </div>
            ...
            <div class="rounded-lg shadow-sm ring-1 ring-black ring-opacity-5">
              <Table>
                ...
                <template v-slot:tbody>
                  <tr
                    v-for="resource in centers.data"
                    :key="resource.uuid"
                  ...
                  >
                    ...
                    <TD>
                      <Link
                        :href="
                          route('centerSemesters.show', {
                            center: resource.uuid,
                          })
                        "
                      >
                      ...
                      <Link
                        :href="route('centers.edit', { centers: resource.uuid })"
                      >
                        <span class="text-indigo-600 hover:text-indigo-900 mr-2"
                          >Edit</span
                        >
      ...
      <!-- Pagination -->
      <Pagination :data="centers.meta"> </Pagination>
    </div>
  </BreezeAuthenticatedLayout>
</template>
