...
function datePicker() {
  Inertia.get(route("rahimia.index", { date: this.date }));
}

function searchPicker() {
  Inertia.get(route("rahimia.index", { search: this.search }));
}
...
function downloadExcelFile() {
  axios({
    ...
    .then((response) => {
      window.location = '/rahimia/download?fileLocation=' + response.data;
    })
    .catch((err) => {
      console.log(err);
    });
    ...
    <div class="">
      <div class="sm:flex sm:items-center">
        ...
        <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
          <Button>
            <Link href="/rahimia/create">Add New</Link>
          </Button>
        </div>
      </div>
            ...
            <div class="rounded-lg shadow-sm ring-1 ring-black ring-opacity-5">
              <Table>
                ...
                <template v-slot:tbody>
                  <tr
                    v-for="resource in rahimia_resources.data"
                    :key="resource.uuid"
                  ...
                  >
                    ...
                    <TD>
                      <Link
                        :href="route('rahimia.edit', { rahimia: resource.uuid })"
                      >
      ...
      <!-- Pagination -->
      <Pagination :data="rahimia_resources.meta"> </Pagination>
    </div>
  </BreezeAuthenticatedLayout>
</template>
